    ...
    extends NeuralQAgent
{
    ...
    {
        super(agentId);
        this.random = new Random(agentId * 17L + 440L);
    }
    ...
    {
        final boolean includeNoAction = game.getAgentInventory(this.agentId()).size() < 5;
        final List<Action> options = this.getRedeemActions(game, actionCounter, canRedeemCards, includeNoAction);
        return chooseRandom(options, this.random);
    }
    ...
    public boolean shouldExploreRedeemMovePhase(final GameView game,
                                                ...
                                                final boolean canRedeemCards)
    {
        return this.random.nextDouble() < this.getExplorationRate(game, 0.28, 0.03);
    }
    ...
    public boolean shouldExploreAttackRedeemIfForcedMovePhase(final GameView game,
                                                              ...
                                                              final boolean canRedeemCards)
    {
        return this.random.nextDouble() < this.getExplorationRate(game, 0.24, 0.02);
    }
    ...
    public boolean shouldExploreFortifySkipMovePhase(final GameView game,
                                                     ...
                                                     final boolean canRedeemCards)
    {
        return this.random.nextDouble() < this.getExplorationRate(game, 0.20, 0.02);
    }
                                               ...
                                               final int remainingArmies)
    {
        if(isDuringSetup)
        {
            return this.random.nextDouble() < this.getExplorationRate(game, 0.32, 0.05);
        }
        return this.random.nextDouble() < this.getExplorationRate(game, 0.18, 0.02);
    }
    ...
    private double getExplorationRate(final GameView game,
                                      ...
                                      final double floor)
    {
        final double turns = game.getNumTurns();
        final double decay = Math.exp(-turns / 2200.0);
