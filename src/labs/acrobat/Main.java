    ...
    extends Object
{
    ...
    {
        NeuralAgent agent = new NeuralAgent(0.01);
        agent.load(filePath);
        return agent;
    }
                                             ...
                                             ReplayBuffer rb) throws Exception
    {
        ...
        for(long episodeIdx = 0; episodeIdx < numEpisodes; ++episodeIdx)
        {
            ...
            while(!done && (t < maxTimeSteps))
            {
                ...
                if(isNeural)
                {
                    if((numMoves > 0) && ((numMoves % updatePeriod) == 0)
                        && (rb.size() > 0))
                    {
                        NeuralAgent neural = (NeuralAgent)agent;
                        Pair<Matrix, Matrix> batchPair =
                            rb.sampleBatch(neural.getQFunction(), gamma, batchSize);
                        Matrix xBatch = batchPair.first();
                        Matrix yScalar = batchPair.second();
                        int rows = xBatch.getShape().numRows();
                        Matrix yGt = Matrix.zeros(rows, 3);
                        for(int r = 0; r < rows; ++r)
                        {
                            double y = yScalar.get(r, 0);
                            yGt.set(r, 0, y);
                            yGt.set(r, 1, y);
                            yGt.set(r, 2, y);
                        }
                        Matrix actionsBatch = Matrix.zeros(rows, 1);
                        for(int u = 0; u < numUpdates; ++u)
                        {
                            neural.update(xBatch, actionsBatch, yGt, lr);
                        }
                    }
